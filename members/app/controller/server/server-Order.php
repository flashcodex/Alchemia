   ...
   public function Fetch_Data()
    {
		...
		$data = array();
		$sql = "SELECT DATE_FORMAT(date_update,'%M %d, %Y %h:%i %p') AS date_approved,(25-days_count) AS days_left,ADDDATE(date_update, INTERVAL 25 DAY) AS days_left2,amount,(SELECT IFNULL(SUM(amount),0.00) FROM tbl_user_commission WHERE type='DAILY_REWARDS' AND comm_from=txnid) AS earned_rewards,txnid FROM tbl_topup_details WHERE status='S' AND user_id='$id' ORDER BY date_update DESC";
					...
					foreach ($result as $row) {
						...
						if($row['days_left']==0){
                     	...
                     	$data[]=array(
                     		...
                     		'earned_rewards' => $row['earned_rewards'],
                     	  );
